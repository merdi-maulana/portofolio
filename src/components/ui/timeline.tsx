          ...
          >
            <div className='flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full'>
              <div className='h-10 absolute left-3 md:left-3 w-10 rounded-full bg-white dark:bg-black flex items-center justify-center'>
                <div
                  className={`h-4 w-4 rounded-full bg-neutral-200 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 p-2 ${index === 0 ? 'bg-primary-200' : ''}`}
                />
              </div>
            </div>

            <div className='relative pl-20 pr-4 md:pl-4 w-full'>{item.content} </div>
          </div>
        ))}
        <div
          style={{
